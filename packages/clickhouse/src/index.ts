...
const getClickHouseClient = (config: ClickHouseClientConfigOptions): ClickHouseClient => {
  ...
  return new Proxy(client, {
    get(target, prop, receiver) {
      if (prop === 'query') {
        return <Format extends QueryParams['format'] = 'JSON'>(
          params: QueryParams & { format?: Format }
        ) =>
          runWithSpan(
            'query',
            params,
      ...
      if (prop === 'exec') {
        return (params: Parameters<ClickHouseClient['exec']>[0]) =>
          runWithSpan('exec', params, connectionAttributes, (nextParams) =>
            target.exec(nextParams)
          );
      }
      ...
      if (prop === 'ping') {
        return (params?: Parameters<ClickHouseClient['ping']>[0]) =>
          runWithSpan('ping', params, connectionAttributes, (nextParams) =>
            target.ping(nextParams)
          );
      }
...
const withTraceHeaders = <TParams extends BaseQueryParams | undefined>(
  params: TParams
): TParams => {
  if (!params) {
    return params;
...
const hasQuery = (
  value: BaseQueryParams | undefined
): value is BaseQueryParams & { query: string } =>
  typeof value === 'object' &&
  value !== null &&
  'query' in value &&
