    ...
    <div class="bg-background border-foreground/10 overflow-hidden rounded-lg border">
        ...
        {#each resources as resource (resource.label)}
          <div>
            <div class="flex items-baseline justify-between">
              <span class="text-muted-foreground text-[11px]">{resource.label}</span>
      ...
      >
        <div class="min-w-0">
          ...
          <p class="text-muted-foreground mt-0.5 truncate text-[11px]">CPU above 90% for 6 min</p>
        </div>
        <span class="flex shrink-0 items-center gap-1.5 text-xs text-amber-700">
          <span class="relative flex size-2">
            <span
              class="absolute inline-flex size-full animate-ping rounded-full bg-amber-500/30 motion-reduce:animate-none"
            ></span>
