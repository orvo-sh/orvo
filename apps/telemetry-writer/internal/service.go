...
func NewService(logger *slog.Logger, natsClient *NATSClient, clickhouseDB *ClickHouse, entitlements *EntitlementsCache, config WriterConfig) *Service {
	return &Service{
		logger:       logger,
		nats:         natsClient,
...
func (service *Service) Run(ctx context.Context) error {
	logsConsumer, err := service.nats.CreateConsumer(ctx, "telemetry-writer-logs", TelemetryLogsSubject)
	if err != nil {
		return err
	}
	tracesConsumer, err := service.nats.CreateConsumer(ctx, "telemetry-writer-traces", TelemetryTracesSubject)
	if err != nil {
		return err
	}
	metricsConsumer, err := service.nats.CreateConsumer(ctx, "telemetry-writer-metrics", TelemetryMetricsSubject)
	if err != nil {
		return err
	}
...
func (service *Service) decodeLogs(ctx context.Context, payload []byte) ([]LogRow, error) {
	...
	entitlement, err := service.entitlements.Get(ctx, message.OrganizationID)
	if err != nil {
		return nil, err
	}
	...
	for _, record := range message.Records {
		...
		expiresAt := ComputeExpiresAt("logs", timestamp, entitlement)
...
func (service *Service) decodeTraces(ctx context.Context, payload []byte) ([]TraceRow, error) {
	...
	for _, span := range message.Spans {
		...
		expiresAt := ComputeExpiresAt("traces", startTime, entitlement)
		if !expiresAt.After(receivedAt) {
			continue
		}
...
func (service *Service) decodeMetrics(ctx context.Context, payload []byte) ([]MetricRow, error) {
	...
	for _, point := range message.Points {
		...
		expiresAt := ComputeExpiresAt("metrics", pointTime, entitlement)
		if !expiresAt.After(receivedAt) {
			continue
		}
...
func ComputeExpiresAt(signal string, eventTime time.Time, entitlement Entitlement) time.Time {
	baseTime := NormalizeTime(eventTime, time.Now().UTC())
	retentionDays := entitlement.RetentionDays(signal)
	if retentionDays <= 0 {
		retentionDays = DefaultLogsRetentionDays
	}
