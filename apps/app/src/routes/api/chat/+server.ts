...
export const POST: RequestHandler = async (event) => {
  ...
  const tools = createOrvoAssistantTools({
    ...
    insightsService: event.locals.container.insightsService,
  });
  const modelId =
    process.env.ORVO_CHAT_MODEL ?? env.ORVO_CHAT_MODEL ?? "gemini-2.5-flash";
  ...
  return result.toUIMessageStreamResponse({
    ...
    onFinish: async ({ messages: finishedMessages, isAborted }) => {
      ...
      if (!saveResult.success) {
        console.error(
          "Ask Orvo failed to save chat messages",
          saveResult.error,
        );
      }
    },
