...
class FilterBuilderState {
    ...
    suggestions = $derived.by(() => {
        ...
        switch (this.builder.stage) {
            case "select_attribute":
                ...
                const attribs = !query ? sorted : sorted.filter((attr) =>
            ...
            case "select_operator":
                ...
                const ops = this.builder.attribute.availableOperators
                    .map((operator) => ({
                        operator,
            ...
            case "select_value":
                if (this.#_expectsMultiValue(this.builder.operator)) {
                    const pickedValues = new Set(this.builder.values.map((value) => value.toLowerCase()));
                    const vals = !query ? this.valueSuggestions : this.valueSuggestions.filter((suggestion) =>
                        suggestion.value.toLowerCase().includes(query),
                    )

                    return vals
                        .filter((suggestion) => !pickedValues.has(suggestion.value.toLowerCase()))
                        .map((suggestion) => ({
                ...
                if (this.#_expectsTypedValue(this.builder.attribute, this.builder.operator)) {
                    return [];
                }
                const vals = !query ? this.valueSuggestions : this.valueSuggestions.filter((suggestion) =>
                    suggestion.value.toLowerCase().includes(query),
    ...
    constructor(
        ...
        $effect(() => {
            if (this.builder.stage !== "select_value" || !this.builder.attribute || !this.builder.operator) {
    ...
    commitFilter = (value: string) => {
        if (!this.builder.attribute || !this.builder.operator || !value.trim()) return;
        this.onAddFilter({
    ...
    applySelectedSuggestion = () => {
        ...
        if (!suggestion) {
            if (this.builder.stage === "select_value" && this.#_expectsMultiValue(this.builder.operator)) {
                if (this.draft.trim()) {
                ...
                if (this.builder.values.length > 0) {
                    this.commitFilter(this.builder.values.join(` ${filterBuilderMultiValueDelimiter} `));
                }
                return;
            }

            if (this.builder.stage === "select_value" && this.draft.trim()) this.commitFilter(this.draft);
            return;
        }
        if (suggestion.kind === "attribute") {
            this.selectAttribute(this.attributes.find(
                (value) => value.key === suggestion.key,
            )!);
            return;
        }
    ...
    stepBack = () => {
        if (this.builder.stage === "select_value") {
            if (this.#_expectsMultiValue(this.builder.operator) && !this.draft && this.builder.values.length > 0) {
                this.builder = {
                    ...this.builder,
