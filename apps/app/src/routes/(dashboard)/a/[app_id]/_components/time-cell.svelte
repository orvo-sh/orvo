...
<div class="flex">
  ...
  {:else if rangeMs <= TWELVE_HOURS_MS}
    ...
    <span class="text-muted-foreground">:{second}</span>
  ...
  {:else if rangeMs <= TWO_WEEKS_MS}
    <span class="mr-1.5 text-muted-foreground">{month}</span>
    <span class="mr-1.5 text-secondary-foreground">{day}</span>
    <span class="text-muted-foreground"> {" "}{hour}:{minute}:{second}</span>
  {:else}
