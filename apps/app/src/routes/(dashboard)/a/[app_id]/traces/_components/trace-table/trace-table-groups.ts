...
const generateTraceTableGroups = (
    traces: Span[],
    ...
    const mode = timeFilter.kind === "preset"
        ? ({
            ...
            last_2_weeks: "day",
            last_month: "day",
        } satisfies Record<TimeFilterPreset, "five_minute" | "quarter_hour" | "hour" | "six_hour" | "day" | "month">)[timeFilter.preset]
        : rangeMs <= HOUR_MS
            ? "five_minute"
            : rangeMs <= 3 * HOUR_MS
                ? "quarter_hour"
    ...
    const groups = new Map<
        string,
        { key: string; label: string; traces: Span[] }
    >();
    ...
    for (const trace of traces) {
        ...
        if (Number.isNaN(date.getTime())) {
            addToGroup({ key: "unknown", label: "Unknown" }, trace);
            continue;
        }
        ...
        if (mode === "five_minute") {
            bucketStart = new Date(year, month, day, hour, Math.floor(minute / 5) * 5);
        ...
        addToGroup({ key, label }, trace);
    }

    return [...groups.values()];
};

export { generateTraceTableGroups };
