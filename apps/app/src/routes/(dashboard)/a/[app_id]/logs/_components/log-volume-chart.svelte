<script lang="ts">
	...
	const SEVERITY_COLORS = {
		...
		debug: 'color-mix(in oklab, var(--color-muted-foreground) 25%, transparent)'
	} as const;
	...
	const generateBarDistribution = ({
		...
		return Array.from({ length: size }, () => {
			...
			const nextValue = previous * smoothness + noise * (1 - smoothness) + spike;
	...
	const getBucketSegments = (bucket: LogVolumeBucket, barHeight: number): ChartSegment[] => {
		if (bucket.total === 0) {
			return [];
		}
	...
	const timeLabelCount = $derived.by(() =>
		Math.max(
			...
			Math.min(
				...
				chartWidth > 0 ? Math.round(chartWidth / TARGET_LABEL_WIDTH_PX) : MAX_TIME_LABEL_COUNT
			)
		)
	);
	...
	const chartBars = $derived.by(() =>
		loading
			...
			: (buckets.map((bucket, index) => ({
					bucket,
					color: bucket.total === 0 ? 'var(--color-border)' : SEVERITY_COLORS.info,
					height:
						bucket.total === 0
							? 1
							: Math.max(
...
<div class="shrink-0 bg-secondary px-4 pt-1 pb-1" bind:clientWidth={chartWidth}>
	<div class="w-full select-none">
		...
		>
						...
						<div class="relative z-10 flex min-w-0 flex-1 items-end justify-center">
							{#if bar.bucket}
								...
								<HoverCard.Root openDelay={50} closeDelay={50}>
									<HoverCard.Trigger
										type="button"
										class={`w-full overflow-hidden rounded-t-[2px] rounded-b-none transition-opacity focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 ${
											bucket.total > 0
									...
									<HoverCard.Content class="w-72 text-sm">
										...
										<div class="flex flex-col gap-0.5">
											...
											{#if bucket.warn > 0}
												<div class="flex items-center gap-1.5">
													<span class="size-2 shrink-0 rounded-sm" style={`background: ${SEVERITY_COLORS.warn};`}></span>
													<span class="text-foreground">
											...
											{#if bucket.info > 0}
												<div class="flex items-center gap-1.5">
													<span class="size-2 shrink-0 rounded-sm" style={`background: ${SEVERITY_COLORS.info};`}></span>
													<span class="text-foreground">{bucket.info} info</span>
												</div>
											{/if}
											{#if bucket.debug + bucket.trace > 0}
												<div class="flex items-center gap-1.5">
													<span class="size-2 shrink-0 rounded-sm" style={`background: ${SEVERITY_COLORS.debug};`}></span>
			...
			<div
				class="mb-1 mt-1 grid"
				style={`grid-template-columns: repeat(${timeLabelCount}, minmax(0, 1fr)); height: ${LABEL_HEIGHT}px;`}
			>
				{#each timeLabels as label}
					<div
						class={`flex min-w-0 overflow-hidden text-[0.75rem] font-mono text-muted-foreground ${label.justifyClass}`}
					>
						<span class="block overflow-hidden text-ellipsis whitespace-nowrap leading-5">
							{label.label}
						</span>
					</div>
				{/each}
