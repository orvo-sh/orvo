...
class LogsPageState {
  ...
  sortBy = $state<LogSortBy>("timestamp");
  sortOrder = $state<LogSortOrder>("desc");
  ...
  constructor(getData: () => PageData) {
    ...
    $effect(() => {
      ...
      void (async () => {
        try {
          const result = await getLogByIdQuery({
