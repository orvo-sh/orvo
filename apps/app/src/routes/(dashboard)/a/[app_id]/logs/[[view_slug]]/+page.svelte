<script lang="ts">
  ...
  const createDefaultViewDefinition = (
    now: Date,
  ): DashboardLogViewDefinition => ({
    version: 1 as const,
    query: {
      time: {
        kind: "range" as const,
  ...
  const getDefinitionSignature = (definition: DashboardLogViewDefinition) =>
  ...
  const createLogStateSearchParams = (
    ...
    if (filters.environments.length > 0) {
      searchParams.set(
        "environment",
        serializeFilterValues(filters.environments),
      );
    }
    return searchParams;
  };
  ...
  const resolveLogStateFromSearchParams = (
    ...
    return {
      ...
      filters: {
        ...
        environments:
          environments.length > 0 ? environments : baseFilters.environments,
  ...
  let defaultViewDefinitionSignature = getDefinitionSignature(
    createDefaultViewDefinition(initialNow),
  );
  ...
  const currentView = $derived(
    views.find((view) => view.slug === currentViewSlug) ?? null,
  );
  const logTimezone = $derived(
    Intl.DateTimeFormat().resolvedOptions().timeZone,
  );
  const createBaseLogState = (
    view: DashboardLogView | null,
    now = new Date(),
  ) =>
    view
      ? {
  ...
  const resolveLogVolumeBucketCount = (start: Date, end: Date) => {
    ...
    const bucketSizeMs =
      AUTO_VOLUME_BUCKET_SIZES_MS.find(
        ({ maxRangeMs }) => rangeMs <= maxRangeMs,
  ...
  const urlStateSearch = $derived(
    createLogStateSearchParams(time, filters).toString(),
  );

  const hasUnsavedViewChanges = $derived.by(() => {
    const currentDefinitionSignature = getDefinitionSignature(
      createCurrentViewDefinition(),
    );
  ...
  const saveAsNewView = async (initialName = "") => {
    ...
    const createdView =
      latestViews?.find((view) => view.id === result.data.id) ?? null;
    if (createdView) {
  ...
  $effect(() => {
    ...
    if (!selectedView && !currentViewSlug && lastAppliedViewId) {
      ...
      live = false;
      lastAppliedViewId = "";
      defaultViewDefinitionSignature = getDefinitionSignature(
        createDefaultViewDefinition(now),
      );
      viewsError = "";
      return;
    }
  ...
  {#snippet actions()}
    ...
    <ButtonGroup.Root>
      <Button
        variant="outline"
        onclick={saveView}
        disabled={!hasUnsavedViewChanges}
      >
      ...
      {#if currentView}
        <DropdownMenu.Root>
          ...
          <DropdownMenu.Content align="end" class="w-44">
            ...
            <DropdownMenu.Item
              onSelect={() => saveAsNewView(`${currentView.name} copy`)}
            >
              <IconDeviceFloppy />
              Save as new view
            </DropdownMenu.Item>
