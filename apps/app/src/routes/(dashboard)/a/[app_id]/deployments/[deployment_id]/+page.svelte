<script lang="ts">
  ...
  const health = $derived(
    result.success ? (result.data as DeploymentHealth) : null,
  );
    ...
    {:else if health && deployment}
        ...
        >
          <div class="min-w-0">
            ...
            <p class="mt-1 truncate text-sm text-muted-foreground">
              {deployment.version ?? "No version"} · started {formatDate(
          ...
          <div
            class="grid gap-2 text-sm text-muted-foreground sm:grid-cols-2 md:min-w-96"
          >
      ...
      <section class="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <div class="rounded-xl border bg-background px-4 py-3">
          <div class="flex items-center justify-between gap-2">
            ...
            >
              ...
              {formatNumber(
                Math.abs(
                  delta(health.before.logs.errors, health.after.logs.errors),
                ),
              )}
            </span>
          </div>
        ...
        <div class="rounded-xl border bg-background px-4 py-3">
          <div class="flex items-center justify-between gap-2">
            ...
            >
              ...
              {formatPercent(
                Math.abs(
                  delta(
                    health.before.traces.errorRate,
                    health.after.traces.errorRate,
                  ),
                ),
              )}
            </span>
          ...
          <p class="mt-1 text-xs text-muted-foreground">
            {health.windowMinutes}m after · {formatPercent(
              health.before.traces.errorRate,
            )} before
          </p>
        </div>
          ...
          <div class="flex items-center justify-between gap-2">
            ...
            >
              ...
              {formatMs(
                Math.abs(
                  delta(
                    health.before.traces.p95LatencyMs,
                    health.after.traces.p95LatencyMs,
                  ),
                ),
              )}
            </span>
        ...
        <section class="rounded-xl border bg-background">
          ...
          >
            <div>
              ...
              <p class="mt-0.5 text-xs text-muted-foreground">
                Top error log bodies in the first {health.windowMinutes} minutes after
                deployment.
              </p>
            </div>
          ...
          <div class="divide-y">
            {#each health.topErrors as errorRow}
              <div
                class="grid gap-3 px-4 py-3 sm:grid-cols-[1fr_96px_140px] sm:items-center"
              >
                ...
                <p class="text-xs text-muted-foreground">
                  {formatDate(errorRow.lastSeenAt)}
                </p>
              </div>
            {:else}
        ...
        <section class="rounded-xl border bg-background">
          ...
          >
            <div>
              ...
              <p class="mt-0.5 text-xs text-muted-foreground">
                Slowest traces after deployment.
              </p>
            </div>
