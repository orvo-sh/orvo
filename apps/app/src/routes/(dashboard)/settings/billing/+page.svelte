<script lang="ts">
	...
	import {
		...
		startOrganizationTrialCommand,
		updateBillingEmailCommand
	} from '$lib/api/billing.remote';
	import { billingSignals } from '$lib/billing';
	...
	let trialPlan = $state<'starter' | 'pro' | null>(null);
	let error = $state('');
	let success = $state('');
	...
	const startTrial = async (plan: 'starter' | 'pro') => {
		trialPlan = plan;
		error = '';
		success = '';

		const result = await startOrganizationTrialCommand({ plan });
		if (result.success === false) {
			error = result.error;
			trialPlan = null;
			return;
	...
	const saveBillingEmail = async () => {
		...
		const result = await updateBillingEmailCommand({ billingEmail: billingEmail.trim() });
		if (result.success === false) {
			error = result.error;
			savingEmail = false;
			return;
			...
			<CardContent class="grid gap-4">
				<div class="grid gap-3 sm:grid-cols-2">
					...
					<div class="rounded-xl border p-4">
						...
						<p class="mt-2 text-lg font-semibold text-foreground">
							{loading ? 'Loading...' : (billingState?.billingEmail ?? 'Not set')}
						</p>
					</div>
				</div>

				<div class="grid gap-3">
					<label class="text-sm font-medium text-foreground" for="billing-email">Billing email</label>
					<Input
						id="billing-email"
					...
					<div class="flex flex-wrap gap-3">
						<Button
							disabled={loading || savingEmail || billingState?.isOwner === false || billingEmail.trim().length === 0}
							loading={savingEmail}
						...
						<Button
							variant="outline"
							disabled={loading || portalLoading || billingState?.isOwner === false}
				...
				<div class="grid gap-3 md:grid-cols-3">
					{#each billingSignals as signal}
						{@const usage = billingState?.usage.find(
							(item: BillingState['usage'][number]) => item.signal === signal
						)}
						<div class="rounded-xl border p-4">
							<p class="text-sm capitalize text-muted-foreground">{signal}</p>
							<p class="mt-2 text-lg font-semibold text-foreground">
								{loading
									? 'Loading...'
		...
		<Card>
			...
			<CardContent class="grid gap-4">
				<p class="text-sm text-muted-foreground">
					If you need custom data retention, volume, or support requirements, talk to the team.
				</p>
				<Button variant="outline" href={`mailto:${billingState?.salesEmail ?? 'team@orvo.sh'}`}>
					Contact sales
				</Button>
			</CardContent>
		</Card>
		...
		{#each billingState?.plans ?? [] as plan (plan.key)}
			<Card class="border-border/80">
				...
				<CardContent class="grid gap-4">
					...
					{:else}
						<Button
							disabled={loading || trialPlan !== null || billingState?.isOwner === false}
							loading={trialPlan === plan.key}
							onclick={() => startTrial(plan.key as 'starter' | 'pro')}
						>
							Start free trial
						</Button>
					{/if}
