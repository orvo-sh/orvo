...
class AlertWebhookDestinationService {
	...
	async testAlertWebhookDestination(
		...
		try {
			...
			const headers = JSON.parse(this.encryption.decrypt(destination.headersEncrypted)) as Array<{
				key: string;
				value: string;
			}>;
			...
			await this.db.transaction(async (tx) => {
				await tx.insert(alertDeliveryAttempt).values({
					...
					errorMessage: response.ok ? null : body.slice(0, 2000) || 'Webhook request failed.'
				});
...
const createAlertWebhookDestinationInputSchema = alertWebhookDestinationInputSchema;

const updateAlertWebhookDestinationInputSchema = alertWebhookDestinationInputSchema.extend({
	id: alertWebhookDestinationIdSchema
});

const deleteAlertWebhookDestinationInputSchema = alertWebhookDestinationIdSchema;
