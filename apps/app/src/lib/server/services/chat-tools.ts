...
const stringListSchema = z.array(z.string().trim().min(1).max(255)).max(25).default([]);
...
const summarizeBuckets = (
	...
	const busiestBucket = buckets.toSorted((a, b) => b.total - a.total)[0] ?? null;
...
const createOrvoAssistantTools = ({
	...
	({
		getAppOverview: tool({
			...
			execute: async () => {
				const appResult = await appService.getApp({ id: app.id }, { organizationId });
		...
		searchTraces: tool({
			...
			inputSchema: z.object({
				...
				services: stringListSchema,
				environments: stringListSchema,
				statusCodes: z.array(z.number().int().min(0).max(255)).max(10).default([]),
				limit: limitSchema
			}),
			execute: async ({ timePreset, search, services, environments, statusCodes, limit }) => {
				...
				return {
					...
					traces: result.data.traces.map((trace) => compactTrace(trace, app.id)),
					nextCursor: result.data.nextCursor,
		...
		getTraceDetails: tool({
			...
			execute: async ({ traceId }) => {
				const result = await tracesService.getTrace({ traceId }, { appId: app.id });
				...
				const services = [...new Set(spans.map((span) => span.service).filter(Boolean))];
				const errorSpans = spans.filter((span) => span.statusCode === 2);
				const slowestSpans = spans.toSorted((a, b) => b.durationMs - a.durationMs).slice(0, 10);
		...
		getSlowTraces: tool({
			...
			execute: async ({ timePreset, services, environments, minDurationMs, limit }) => {
				const result = await tracesService.getTraces(
					{
						time: toTimeFilter(timePreset),
						search: '',
		...
		getAlertCoverage: tool({
			...
			execute: async ({ timePreset }) => {
				const [rulesResult, volumeResult, errorsResult, slowTracesResult] = await Promise.all([
				...
				if ((volume.totals.error > 0 || volume.totals.fatal > 0) && !signals.has('error_rate')) {
					suggestions.push({
						type: 'error_rate',
						reason: 'Recent error or fatal logs exist, but there is no error-rate rule.'
					});
				}

				if (slowTraces.some((trace) => trace.durationMs >= 1000) && !signals.has('latency_p95_ms')) {
					suggestions.push({
						type: 'latency_p95_ms',
						reason: 'At least one recent trace is over 1s, but there is no p95 latency rule.'
					});
				}
				...
				if (!signals.has('throughput_per_min')) {
					suggestions.push({
						...
						reason: 'A throughput rule helps detect sudden drops or spikes in traffic.'
					});
				}
				...
				return {
					...
					telemetry: {
						logVolume: volume,
						recentErrors: errorsResult.data.logs.map((log) => compactLog(log, app.id)),
						slowTraces
					},
					suggestions,
