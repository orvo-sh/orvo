...
class AppService {
	...
	async getApp(input: z.infer<typeof getAppInputSchema>, context: { organizationId: string }) {
	...
	async updateAppSettings(
		...
		this.logger.info('updateAppSettings: updating app settings', { input, context });
		...
		try {
			const existing = await this.db.query.app.findFirst({
				where: and(eq(app.id, context.appId), eq(app.organizationId, context.organizationId))
			});
			...
			this.logger.error('updateAppSettings: failed to update app settings', error as Error);
...
export { AppService, createAppInputSchema, getAppInputSchema, updateAppSettingsInputSchema };
