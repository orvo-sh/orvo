...
const kbdShortcut = (callbacks: Record<string, () => any>) => {
    ...
    const cb = (event: KeyboardEvent) => {
        ...
        if (!targetElement?.tagName || ignoredElements.has(targetElement.tagName)) return;
...
const ignoredRoles = new Set([
    'dialog', 'menu', 'listbox', 'option', 'menuitem', 'menuitemcheckbox', 'menuitemradio', 'radiogroup', 'tablist', 'tree', 'treeitem'
])
