...
export const getIngestionKeyQuery = query(getIngestionKeyInputSchema, async (input) => {
...
export const rotateIngestionKeyCommand = command(rotateIngestionKeyInputSchema, async (input) => {
	const event = getRequestEvent();
	const appContext = await resolveRequestAppContext(event);
