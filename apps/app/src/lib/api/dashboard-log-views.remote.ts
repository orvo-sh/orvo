...
export const createDashboardLogViewCommand = command(createDashboardLogViewInputSchema, async (input) => {
	const event = getRequestEvent();
	const appContext = await resolveRequestAppContext(event);
...
export const updateDashboardLogViewCommand = command(updateDashboardLogViewInputSchema, async (input) => {
	const event = getRequestEvent();
	const appContext = await resolveRequestAppContext(event);
...
export const deleteDashboardLogViewCommand = command(deleteDashboardLogViewInputSchema, async (id) => {
	const event = getRequestEvent();
	const appContext = await resolveRequestAppContext(event);
